    ...
    <div class="container mb-20 max-w-screen-lg mx-auto">
        ...
        <form method="POST" action="{{ route('licor.update',$licor->id) }}">
            ...
            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
                ...
                <input name="nombre" type="text" id="nombre" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('nombre') ?? $licor->nombre}}" />
                    @error('nombre')
                    ...
                    <input type="text" name="descripcion" id="descripcion" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{ old('descripcion') ?? $licor->descripcion }}" placeholder="Descripcion del producto" />
                        @error('descripcion')
                            <p style="color: red;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <input type="number" name="precio" id="precio" class="transition-all flex items-center h-10 border mt-1 rounded px-4 w-full bg-gray-50" placeholder="" value="{{ old('precio') ?? $licor->precio }}" />
                        @error('precio')
                            <p style="color: red;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-1">
                        ...
                        <input type="number" name="stock" id="stock" class="transition-all flex items-center h-10 border mt-1 rounded px-4 w-full bg-gray-50" placeholder="" value="{{ old('stock') ?? $licor->stock }}" />
                            @error('stock')
                                <p style="color: red;">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="md:col-span-1">
                        ...
                        <select type="text" name="categoria_id" id="categoria_id" class="transition-all flex items-center h-10 border mt-1 rounded px-4 w-full bg-white" placeholder="">
                            @foreach($categorias as $categoria)
                                <option  value="{{ $categoria->id }}" {{ $categoria->id == $licor->categoria_id ? 'selected' : '' }} @selected(old('categoria_id') == $categoria->id)>
                                    {{ $categoria->nombre }}
                                </option>
                            @endforeach
                        </select>
